 ...
 * 
 * <p>This class provides methods to compare cards based on their functional properties,
 * allowing the AI to group identical cards together and evaluate them once instead of
 * individually. This significantly improves performance when dealing with multiple
 * copies of the same card or tokens.</p>
 * 
 * <h3>Equality Types:</h3>
 * <ul>
 *   <li><b>Functional Equality:</b> Cards that behave identically from a gameplay perspective.
 *       Ignores cosmetic properties like art index and foil status.</li>
 *   <li><b>Strict Equality:</b> Functional equality plus timestamp comparison. Use when
 *       temporal order matters (e.g., for layering effects).</li>
 *   <li><b>PaperCard Functional Equality:</b> For deck-building decisions, comparing cards
 *       by name and functional variant only.</li>
 * </ul>
 */
public class CardEqualityUtil {
    
    /**
     * Compares two cards for functional equality.
     * 
     * <p>Two cards are functionally equal if they behave identically in the game.
     * This comparison includes:</p>
     * <ul>
     *   <li>Card name</li>
     ...
     * </ul>
     * 
     * <p>This comparison ignores cosmetic properties like art index, foil status,
     * collector number, and timestamp.</p>
     * 
     ...
     */
    public static boolean areFunctionallyEqual(Card card1, Card card2) {
        if (card1 == card2) {
            return true;
        }
        ...
        // Compare keywords (simplified - just check if they have the same keywords)
     ...
     */
    public static boolean areStrictlyEqual(Card card1, Card card2) {
     ...
     * 
     * <p>For deck-building and card selection AI, this compares cards based on:</p>
     * <ul>
     *   <li>Card name</li>
     *   <li>Functional variant (e.g., rebalanced versions)</li>
     * </ul>
     * 
     * <p>This ignores edition, art index, foil status, and collector number.</p>
     * 
     ...
     */
    public static boolean arePaperCardsFunctionallyEqual(PaperCard card1, PaperCard card2) {
        if (card1 == card2) {
            return true;
        }
        if (card1 == null || card2 == null) {
        ...
        return Objects.equals(card1.getFunctionalVariant(), card2.getFunctionalVariant());
    }
     ...
     */
    private static String buildFunctionalKey(Card card) {
        if (card == null) {
            return "null";
        }
        ...
        // Add controller ID
        key.append(card.getController() != null ? card.getController().getId() : "null").append("|");
        ...
        if (card.isCreature()) {
            key.append(card.getNetPower()).append("/").append(card.getNetToughness()).append("|");
        } else {
            key.append("N/A|");
        }
     ...
     * where all cards in each bucket are functionally identical. The map returned
     * uses a representative card from each group as the key.</p>
     * 
     * <p>Example usage:</p>
     * <pre>
     * Map&lt;Card, List&lt;Card&gt;&gt; groups = CardEqualityUtil.groupByFunctionalEquality(cardList);
     ...
     * @return a map where keys are representative cards and values are lists of
     *         functionally identical cards (including the representative)
     */
    public static Map<Card, List<Card>> groupByFunctionalEquality(Collection<Card> cards) {
     ...
     * <p>Example usage:</p>
     * <pre>
     * Map&lt;Card, Integer&gt; counts = CardEqualityUtil.getBatchableDecisions(cardList);
     ...
     */
    public static Map<Card, Integer> getBatchableDecisions(Collection<Card> cards) {
